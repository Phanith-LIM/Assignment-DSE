...
class Teacher:
    ...
    def add_teacher(self):
        ...
        # select gender menu
        list_gender = ['M', 'F']
        gender = list_gender[TerminalMenu(list_gender, title="Gender: ", menu_highlight_style=("fg_red", "fg_red", "bold")).show()]
        print('Gender: ' + gender)
    ...
    def update_teacher(self, teacherId):
        ...
        # select gender
        list_gender = ['M', 'F']
        self.teacher_data[teacherId]['gender'] = list_gender[
            TerminalMenu(list_gender, title="Gender: ", menu_highlight_style=("fg_red", "fg_red", "bold")).show()]
        print('Gender: ' + self.teacher_data[teacherId]['gender'])
    ...
    def delete_teacher(self, teacherId):
        ...
        if confirm == 'No':
            ...
            return
        with open(self.det_path, 'a', newline='') as fileDelete, open(self.teacher_course_path, 'w', newline='') as fileTC:
...
if __name__ == '__main__':
    teacher = Teacher()
    while True:
        list_menu = ['[a]Add new a Teacher', '[b]Search a Teacher by ID ', '[c]Update a Teacher', '[d]Delete a Teacher',
                     '[e]Display all courses taught by a teacher.', '[f]Exit']
        menu = TerminalMenu(list_menu, title='[Teacher Management]',
                            menu_highlight_style=('fg_blue', "fg_blue", "bold")).show()
        if menu == 0:
            teacher.add_teacher()
        elif menu == 1:
